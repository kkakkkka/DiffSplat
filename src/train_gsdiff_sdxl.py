...
from extensions.diffusers_diffsplat import MyEMAModel, UNetMV2DConditionModel, StableMVDiffusionXLPipeline


@torch.no_grad()
def log_validation(
    dataloader, negative_prompt_embed, negative_pooled_prompt_embed, lpips_loss, gsrecon, gsvae, vae, unet,
    ...
    pipeline = StableMVDiffusionXLPipeline(
...
def main():
    ...
    if args.load_pretrained_model is None:
        ...
        logger.info(f"Load MVUNet EMA checkpoint from [{args.load_pretrained_model}] iteration [{args.load_pretrained_model_ckpt:06d}]\n")
    ...
    if args.use_ema:
        ema_unet = MyEMAModel(
            unet.parameters(),
            model_cls=UNetMV2DConditionModel,
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                load_model = MyEMAModel.from_pretrained(os.path.join(input_dir, "unet_ema"), UNetMV2DConditionModel)
    ...
    unet, optimizer, lr_scheduler, train_loader, val_loader = accelerator.prepare(
        unet, optimizer, lr_scheduler, train_loader, val_loader
    )
    ...
    if accelerator.is_main_process:
        ...
        util.save_model_architecture(accelerator.unwrap_model(unet), exp_dir)
    ...
    for batch in yield_forever(train_loader):
        ...
        with accelerator.accumulate(unet):
            ...
            if opt.view_concat_condition:
                with torch.no_grad():
                    ...
                    image_latents = vae.config.scaling_factor * vae.encode(imgs_cond.to(vae_weight_dtype) * 2. - 1.).latent_dist.sample()  # (B*V_cond, 4, H', W')
            ...
            pooled_prompt_embeds = repeat(pooled_prompt_embeds, "b d -> (b v) d", v=V_in + (V_cond if opt.view_concat_condition else 0))
            ...
            time_ids = repeat(time_ids, "d -> (b v) d", b=B, v=V_in + (V_cond if opt.view_concat_condition else 0))
        ...
        if accelerator.sync_gradients:
            ...
            if args.use_ema:
                ...
                logs.update({"ema": ema_unet.cur_decay_value})
            if use_rendering_loss:
