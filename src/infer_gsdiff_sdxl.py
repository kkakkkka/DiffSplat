...
from extensions.diffusers_diffsplat import UNetMV2DConditionModel, StableMVDiffusionXLPipeline, MVControlNetModel, StableMVDiffusionControlNetPipeline
...
def main():
    ...
    if args.prompt_file is not None:
        with open(args.prompt_file, "r") as f:
            prompts = prompts_2 = [line.strip() for line in f.readlines() if line.strip() != ""]
        negative_prompt = negative_prompt_2 = args.negative_prompt.replace("_", " ")
        if negative_prompt == "":  # reset negative prompt for SDXL
            negative_prompt = negative_prompt_2 = None
        negative_promts, negative_promts_2 = \
            [negative_prompt] * len(prompts), [negative_prompt_2] * len(prompts_2)
    else:
        prompt = prompt_2 = args.prompt.replace("_", " ")
        negative_prompt = negative_prompt_2 = args.negative_prompt.replace("_", " ")
        if negative_prompt == "":  # reset negative prompt for SDXL
            negative_prompt = negative_prompt_2 = None
        prompts, prompts_2, negative_promts, negative_promts_2 = \
    ...
    for i in range(len(image_paths)):  # to save outputs with the same name as the input image
        ...
        for j in range(len(prompts)):
            prompt, prompt_2, negative_prompt, negative_prompt_2 = \
                prompts[j], prompts_2[j], negative_promts[j], negative_promts_2[j]
