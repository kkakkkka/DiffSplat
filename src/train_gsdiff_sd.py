...
from extensions.diffusers_diffsplat import MyEMAModel, UNetMV2DConditionModel, StableMVDiffusionPipeline


@torch.no_grad()
def log_validation(
    dataloader, negative_prompt_embed, lpips_loss, gsrecon, gsvae, vae, unet,
        ...
        with accelerator.accumulate(unet):
            ...
            model_pred = unet(
                ...
                cross_attention_kwargs=dict(
                    num_views=V_in + (V_cond if opt.view_concat_condition else 0),
                ),
            ).sample
