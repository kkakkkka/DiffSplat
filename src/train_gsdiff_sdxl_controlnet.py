...
from extensions.diffusers_diffsplat import MyEMAModel, UNetMV2DConditionModel, MVControlNetModel, StableMVDiffusionXLControlNetPipeline


@torch.no_grad()
def log_validation(
    dataloader, negative_prompt_embed, negative_pooled_prompt_embed, lpips_loss, gsrecon, gsvae, vae, unet, controlnet,
    ...
    pipeline = StableMVDiffusionXLControlNetPipeline(
        text_encoder=None, tokenizer=None,
        text_encoder_2=None, tokenizer_2=None,
        vae=vae, unet=unet, controlnet=accelerator.unwrap_model(controlnet),
    ...
    for i, batch in enumerate(dataloader):
        imgs_in = batch[opt.controlnet_type][:, 0, ...]  # the first view as input
        B, V_in, V = imgs_in.shape[0], opt.num_input_views, opt.num_views
        ...
        with torch.autocast("cuda", torch.bfloat16):
            for guidance_scale in sorted(args.val_guidance_scales):
                out = pipeline(
                    imgs_in, num_inference_steps=opt.num_inference_steps, guidance_scale=guidance_scale,
...
def main():
    ...
    parser.add_argument(
        "--load_pretrained_gsdiff",
        type=str,
        default="gsdiff_gobj83k_sdxl__render",
        help="Tag of a pretrained MVUNet in this project"
    )
    parser.add_argument(
        "--load_pretrained_gsdiff_ckpt",
        type=int,
        default=13020,
        help="Iteration of the pretrained MVUNet checkpoint"
    )
    parser.add_argument(
        "--load_pretrained_model",
        type=str,
        default=None,
        help="Tag of a pretrained MVUNet ControlNet in this project"
    )
    parser.add_argument(
        "--load_pretrained_model_ckpt",
        type=int,
        default=-1,
        help="Iteration of the pretrained MVUNet ControlNet checkpoint"
    )

    # Parse the arguments
    args, extras = parser.parse_known_args()
    args.val_guidance_scales = [float(x[0]) if isinstance(x, list) else float(x) for x in args.val_guidance_scales]
    args.use_ema = False  # NOTE: NOT USE EMA for CONTROLNET
    ...
    in_channels = 4  # hard-coded for SDXL
    if opt.input_concat_plucker:
        in_channels += 6
    if opt.input_concat_binary_mask:
        in_channels += 1
    unet_from_pretrained_kwargs = {
        "sample_size": opt.input_res // 8,  # `8` hard-coded for SDXL
    ...
    if args.load_pretrained_gsdiff is None:
        ...
        logger.info(f"Load MVUNet checkpoint from [{args.load_pretrained_gsdiff}] iteration [{args.load_pretrained_gsdiff_ckpt:06d}]\n")
        path = f"out/{args.load_pretrained_gsdiff}/checkpoints/{args.load_pretrained_gsdiff_ckpt:06d}"
        if not os.path.exists(path):
            if accelerator.is_local_main_process:
                os.system(f"python3 extensions/download_ckpt.py {args.load_pretrained_gsdiff} {args.load_pretrained_gsdiff_ckpt:06d}")
    ...
    if args.load_pretrained_model is None:
        logger.info(f"Initializing ControlNet weights from MVUNet")
        controlnet = MVControlNetModel.from_unet(unet, conditioning_channels=opt.controlnet_input_channels)
    else:
        logger.info(f"Load MVUNet ControlNet EMA checkpoint from [{args.load_pretrained_model}] iteration [{args.load_pretrained_model_ckpt:06d}]\n")
        ...
        controlnet = MVControlNetModel.from_unet(unet, conditioning_channels=opt.controlnet_input_channels)
        load_checkpoint_and_dispatch(controlnet, os.path.join(path, "controlnet/diffusion_pytorch_model.safetensors"))
    ...
    # controlnet.enable_xformers_memory_efficient_attention()  # use `tF.scaled_dot_product_attention` instead
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            for _ in range(len(models)):
                ...
                load_model = MVControlNetModel.from_unet(unet, conditioning_channels=opt.controlnet_input_channels)
                load_checkpoint_and_dispatch(load_model, os.path.join(input_dir, "controlnet/diffusion_pytorch_model.safetensors"))
    ...
    controlnet, optimizer, lr_scheduler, train_loader, val_loader = accelerator.prepare(
        controlnet, optimizer, lr_scheduler, train_loader, val_loader
    )
    # Set classes explicitly for everything
    controlnet: DistributedDataParallel
    ...
    if accelerator.mixed_precision == "fp16":
        ...
        vae_weight_dtype = torch.bfloat16
    elif accelerator.mixed_precision == "bf16":
        vae_weight_dtype = weight_dtype = torch.bfloat16

    # NOTE: Move `gsrecon`, `vae` and `gsvae` to gpu and cast to `vae_weight_dtype` to avoid numerical problems of SDXL
    gsrecon.to(accelerator.device, dtype=vae_weight_dtype)
    vae.to(accelerator.device, dtype=vae_weight_dtype)
    gsvae.to(accelerator.device, dtype=vae_weight_dtype)
    ...
    if accelerator.is_main_process:
        ...
        util.save_model_architecture(accelerator.unwrap_model(controlnet), exp_dir)
    ...
    for batch in yield_forever(train_loader):
        ...
        with accelerator.accumulate(controlnet):
            ...
            (
                imgs_in, prompt_embeds, negative_prompt_embeds,
            ...
            imgs_in = torch.stack([imgs_in] + [torch.ones_like(imgs_in)] * (V_in - 1), dim=1)  # (B, V_in, 3, H, W)
            imgs_in = rearrange(imgs_in, "b v c h w -> (b v) c h w")
            ...
            with torch.no_grad():
                latents = gsvae.scaling_factor * (gsvae.get_gslatents(gsrecon,
                    imgs_out.to(vae_weight_dtype), input_C2W.to(vae_weight_dtype), input_fxfycxcy.to(vae_weight_dtype)) - gsvae.shift_factor)  # (B*V_in, 4, H', W')
            ...
            prompt_embeds = repeat(prompt_embeds, "b n d -> (b v) n d", v=V_in)
            pooled_prompt_embeds = repeat(pooled_prompt_embeds, "b d -> (b v) d", v=V_in)
            ...
            if opt.input_concat_plucker:
                ...
                plucker = rearrange(plucker, "(b v) c h w -> b v c h w", v=V_in)
                latent_model_input = torch.cat([latent_model_input, plucker], dim=2)  # (B, V_in(+1), 4+6, H', W')
                plucker = rearrange(plucker, "b v c h w -> (b v) c h w")
            if opt.input_concat_binary_mask:
            ...
            # SDXL time id conditions (not really used)
            time_ids = torch.tensor([
                opt.input_res, opt.input_res,  # original_size
                0, 0,                          # crops_coords_top_left
                opt.input_res, opt.input_res,  # target_size
            ], dtype=weight_dtype, device=latents.device)
            time_ids = repeat(time_ids, "d -> (b v) d", b=B, v=V_in)
            ...
            down_block_res_samples, mid_block_res_sample = controlnet(
                ...
                cross_attention_kwargs=dict(
                    num_views=opt.num_input_views,
                ),
                added_cond_kwargs=added_cond_kwargs,
                controlnet_cond=imgs_in,
                return_dict=False,
            )
            ...
            model_pred = unet(
                ...
                down_block_additional_residuals=[
                    sample.to(weight_dtype) for sample in down_block_res_samples
                ],
            ...
            if use_rendering_loss:
                ...
                pred_render_outputs = gsvae.decode_and_render_gslatents(gsrecon,
                    pred_original_latents.to(vae_weight_dtype), input_C2W.to(vae_weight_dtype), input_fxfycxcy.to(vae_weight_dtype),
                    C2W.to(vae_weight_dtype), fxfycxcy.to(vae_weight_dtype),
