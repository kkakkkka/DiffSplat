...
from extensions.diffusers_diffsplat import MyEMAModel, PixArtTransformerMV2DModel, PixArtAlphaMVPipeline
...
def main():
    ...
    if not opt.edm_style_training:
        noise_scheduler = DDPMScheduler.from_pretrained("PixArt-alpha/PixArt-XL-2-512x512", subfolder="scheduler")
    else:
    ...
    for batch in yield_forever(train_loader):
        ...
        with accelerator.accumulate(transformer):
            ...
            model_pred = model_pred.chunk(2, dim=1)[0]  # hard-coded for PixArt-alpha
