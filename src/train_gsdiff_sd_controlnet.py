...
from extensions.diffusers_diffsplat import MyEMAModel, UNetMV2DConditionModel, MVControlNetModel, StableMVDiffusionControlNetPipeline


@torch.no_grad()
def log_validation(
    dataloader, negative_prompt_embed, lpips_loss, gsrecon, gsvae, vae, unet, controlnet,
...
def main():
    ...
    if args.load_pretrained_gsdiff is None:
        ...
        logger.info(f"Load MVUNet EMA checkpoint from [{args.load_pretrained_gsdiff}] iteration [{args.load_pretrained_gsdiff_ckpt:06d}]\n")
    ...
    if args.load_pretrained_model is None:
        ...
        logger.info(f"Load MVUNet ControlNet checkpoint from [{args.load_pretrained_model}] iteration [{args.load_pretrained_model_ckpt:06d}]\n")
    ...
    # Move `gsrecon`, `vae`, `gsvae` and `unet` to gpu and cast to `weight_dtype`
    ...
    for batch in yield_forever(train_loader):
        ...
        with accelerator.accumulate(controlnet):
            ...
            imgs_in, prompt_embeds, negative_prompt_embeds, imgs_out, C2W, fxfycxcy = (
